...
from agent.evals.types import EvalInputs, EvalMetadata, NeighborEvalTask
from agent.neighbor import NeighborAnalysisResult
from agent.neighbor_single.agent import (
...
async def run_neighbor_single_eval(
    inputs: EvalInputs,
    ...
    try:
        async with eval_environment(
            ...
            return await run_neighbor_single_agent_analysis(
                save_filename=inputs["save_filename"],
                settings=eval_settings,
                model_name=model_name,
            )
    except Exception as e:
        logfire.error(f"Neighbor single eval failed: {e!r}")
        raise


def create_neighbor_single_eval_task(
    model_name: str | None = None,
    ...
    async def eval_task(
        ...
        return await run_neighbor_single_eval(
            inputs,
            model_name=model_name,
            settings=settings,
        )
...
async def run_neighbor_single_evals(
    dataset: Dataset[EvalInputs, NeighborAnalysisResult, EvalMetadata],
    model_name: str | None = None,
    experiment_name: str | None = None,
    settings: Settings | None = None,
) -> EvaluationReport[EvalInputs, NeighborAnalysisResult, EvalMetadata]:
    logfire.configure(send_to_logfire="if-token-present")
    logfire.instrument_pydantic_ai()
    logfire.instrument_httpx()

    task = create_neighbor_single_eval_task(
        model_name,
        experiment_name,
        settings,
    )
